...
def fullmenu(request):
    snacks = Item.objects.filter(labels = "Snacks")
    mc = Item.objects.filter(labels = "Main Course")
    bd = Item.objects.filter(labels = "Drinks & Beverages")
    context = {
        'snacks': snacks,
        'mc': mc,
        'bd': bd,
    }
    return render(request, 'main/fullmenu.html', context)
...
def payment_done(request,pk):
    ...
    cart_items.update(status="Active-payment-done", delivery_date=delivery_date)
